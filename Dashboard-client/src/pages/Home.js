...
export default function Home() {
  ...
  // Returning the JSX for the Home component matched to the black theme
  return (
    <div
      style={{
    ...
    >
      ...
      <MDBContainer className="p-1">
        <MDBTypography
          ...
          className="display-3 text-center fw-bold mb-4"
        >
          Welcome {user?.name}!
        </MDBTypography>
        ...
        <MDBContainer
          fluid
          className="d-flex align-items-center mt-3 mb-3 gap-2"
        >
          <Sort reversed={reversed} setReversed={setReversed} />
          <Filter
            {...{ colors, addColor, removeColor, types, addType, removeType }}
          />
        ...
        {(error || todoListError) && (
          <Error error={error ? error : todoListError} />
        )}
        ...
        >
          <MDBRow className="">
            {allElements.length === 0 && !loading && (
              <NoElements {...{ colors, types, removeColor, removeType }} />
            )}
